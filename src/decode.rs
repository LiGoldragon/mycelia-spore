...
fn map_site_kind(kind: spore_capnp::site_kind::Reader) -> Result<SiteKind> {
...
fn map_hosting_provider(p: spore_capnp::hosting_provider::Reader) -> Result<HostingProvider> {
...
fn map_repo_provider(p: spore_capnp::repo_provider::Reader) -> Result<RepoProvider> {
...
fn map_build_type(t: spore_capnp::build_type::Reader) -> Result<BuildType> {
...
fn map_cf_framework(f: spore_capnp::cf_framework::Reader) -> Result<CfFramework> {
...
fn map_registrar(r: spore_capnp::purchase_config::registrar::Reader) -> Result<Registrar> {
...
pub fn decode_spore(reader: spore_config::Reader) -> Result<SporeConfig> {
    ...
    let site = Site {
        ...
        kind: map_site_kind(site_reader.get_kind()?)?,
    };

    let repo = Repo {
        provider: map_repo_provider(repo_reader.get_provider()?)?,
        slug: repo_reader.get_slug()?.to_string(),
        default_branch: repo_reader.get_default_branch()?.to_string(),
    };

    let build = Build {
        build_type: map_build_type(build_reader.get_type()?)?,
        output_dir: build_reader.get_output_dir()?.to_string(),
        framework: map_cf_framework(build_reader.get_framework()?)?,
    };
    ...
    let hosting = Hosting {
        provider: map_hosting_provider(hosting_reader.get_provider()?)?,
    ...
    let dns = if reader.has_dns() {
        ...
        Some(DnsConfig {
            provider: map_hosting_provider(dns_reader.get_provider()?)?,
    ...
    let purchase = if reader.has_purchase() {
        ...
        Some(PurchaseConfig {
            provider: map_registrar(p.get_provider()?)?,
...
pub fn spore_stream() -> impl Iterator<Item = Result<SporeConfig>> {
    let stdin = io::stdin();
...
struct SporeIterator<R: io::Read> {
    reader: BufReader<R>,
    opts: ReaderOptions,
}

impl<R: io::Read> Iterator for SporeIterator<R> {
